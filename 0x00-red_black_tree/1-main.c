...
/**
 * invalid_rb_1 - Builds a valid R-B tree
 *
 * Return: A pointer to the created tree
 */
rb_tree_t *invalid_rb_1(void)
{
	rb_tree_t *root;

	root = rb_tree_node(NULL, 98, BLACK);
	root->right = rb_tree_node(root, 90, BLACK);
	root->right->right = rb_tree_node(root->right, 120, BLACK);
	return (root);
}
...
rb_tree_t *valid_rb_1(void)
{
	...
	root = rb_tree_node(NULL, 98, BLACK);
	root->right = rb_tree_node(root, 90, BLACK);
	root->right->right = rb_tree_node(root->right, 120, RED);
	root->left = rb_tree_node(root, 79, BLACK);
	return (root);
}
